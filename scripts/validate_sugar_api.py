...
def held(state):
    ...
    assert state['taste']['body_held'] and not state['taste']['vision_and_odor_input']
    if state['brain']:
...
print('Sugar presentation, held body, pause/resume and ownership passed', flush=True)
...
result = {'passed':True,'arrival_time':arrival['time'],
          'checks':['arrival required','invalid rate preserves trial','shared ownership',
                    'pause and resume','held body and independent assay clock',
                    'downstream MN9 response','silent no-taste control','reset clears assay'],
